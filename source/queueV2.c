...
int queue_check_if_drive_further(QueueState2 * p_queue) {
    if (p_queue->preferred_motor_state == HARDWARE_MOVEMENT_UP) {
        for (int i = 0; i < HARDWARE_NUMBER_OF_FLOORS; i++) {
            if (i > p_queue->saved_floor) {
                if (p_queue->order_up[i] == 1) {
                    p_queue->further_floor = i;
                    return 1;
                }
    ...
    if (p_queue->further_floor == p_queue->saved_floor) {
        p_queue->further_floor = -1;
    }
    return 0;
}
...
int queue_check_if_stop_floor(QueueState2* p_queue) {
    if(p_queue->current_floor >=0) {
        if(
            p_queue->further_floor == p_queue->saved_floor
